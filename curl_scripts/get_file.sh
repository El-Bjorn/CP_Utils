...
#		usage: ./get_file.sh token file_id
if (( $# != 2)); then
	echo "Wrong number of args. Usage: ./get_file.sh token file_id"
	exit
fi
...
echo "Getting CoPatient file with ID:$2 ...." 
...
get_URI="/api/file/get"

get_request=$get_URI'?access_key='$access_key'&token='$auth_token'&file_id='$file_id

echo "get_request = $get_request"

#exit

raw_hash_value=$(echo -n $get_request | $openssl sha256 -hmac $secret_key)
